// sale-ready.js - Sale-Ready Scan free-scan form. Validation, submit, UI states.
export function isValidEmail(s){
  return typeof s === 'string' && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(s.trim());
}
export function buildPayload(email, note){
  return { email: (email||'').trim(), note: '[Sale-Ready] ' + (note||'').trim() };
}
...
if (typeof document !== 'undefined'){
  ...
  if (form){
    const emailEl = document.getElementById('sale-email');
    const noteEl = document.getElementById('sale-note');
    const hpEl = document.getElementById('sale-hp');
